...
config_single(){
	...
	tc qdisc add dev h${1}-eth${2} handle 2: parent 1: tbf burst 80KB latency ${3} mtu 1514 rate ${4}Mbit
...
format is like ./host_config.sh (h)i (-eth)j latency=3(ms) rate=1(Mbit/s)"
fi